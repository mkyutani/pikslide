#!/usr/bin/env bash
# Render a .pptx to a PNG of its (first, and normally only -- pikslide's
# own output is always one slide) slide.
#
# Prefers PowerPoint COM automation (pptx_to_png.ps1, beside this script)
...
#
# Usage: scripts/pptx_to_png.sh INPUT.pptx OUTPUT.png

set -euo pipefail

if [ "$#" -ne 2 ]; then
    echo "usage: $0 INPUT.pptx OUTPUT.png" >&2
    exit 2
fi
...
if command -v powershell.exe >/dev/null 2>&1; then
    ...
    if command -v wslpath >/dev/null 2>&1; then
        ...
        win_script=$(wslpath -w "$script_dir/pptx_to_png.ps1")
    else
        win_in=$in
        win_out=$out
        win_script="$script_dir/pptx_to_png.ps1"
    fi
    if powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$win_script" \
        -PptxPath "$win_in" -PngPath "$win_out"; then
        exit 0
    fi
...
out_dir=$(dirname "$out")
soffice --headless --convert-to png --outdir "$out_dir" "$in" >/dev/null
# soffice names its output after INPUT's own stem, not OUTPUT -- rename.
produced="$out_dir/$(basename "${in%.*}").png"
